...
int readImage(image_desc *pDesc, targa_header *pHead, char * fName){
  ...
  /* Open fDesc file */
  fDesc = fopen(fName,"r");
  printf("readImage : Return from fopen : %d \n",fDesc);
  /* Lecture du header */
  ...
  if (DEBUG_MODE == 1){
    ...
    printf("Origine de l'image : %u %u",pHead->origins[0],pHead->origins[1]);
  }
...
int writeImage (image_desc iDesc, targa_header head, char * fName){
  ...
  /* Variables temporaires uniquement pour faciliter la lecture du code */
  pBlue = iDesc.pBlue;
  pGreen = iDesc.pGreen;
  pRed = iDesc.pRed;
  ...
  for (i=0; i<head.width*head.height; i++)
  {
    fwrite(pBlue+i, sizeof(uint8_t), 1, fDesc); // pBlue+i equiv. a &((*pBlue)[i])
    fwrite(pGreen+i, sizeof(uint8_t), 1, fDesc);
    fwrite(pRed+i, sizeof(uint8_t), 1, fDesc);
  }
  fclose (fDesc);
  printf("Wrote image in file %s\n", fName);
  return 1;
}
